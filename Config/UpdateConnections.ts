...
import {ConnectionConfigError} from './ConnectionConfigError.js';

export {ConnectionConfigError} from './ConnectionConfigError.js';
export type {ConnectionConfigErrorCode} from './ConnectionConfigError.js';
...
export class ConnectionConfig {
     ...
     * `ProjectConfig.add`, so the lookup is unambiguous.
     */
    private static _findProjectIndex(config: Config, projectName: string): number {
     ...
     * codebase).
     */
    public static add(rawConfig: unknown, projectName: string, input: AddConnectionInput): Config {
        const config = ConnectionConfig._validateConfig(rawConfig);
        const idx = ConnectionConfig._findProjectIndex(config, projectName);
        if (idx < 0) {
        ...
        ConnectionConfig._revalidateMerged(nextConfig);
        return nextConfig;
    }
     ...
     */
    public static remove(rawConfig: unknown, projectName: string, databaseUnid: string): Config {
        const config = ConnectionConfig._validateConfig(rawConfig);
        const idx = ConnectionConfig._findProjectIndex(config, projectName);
        if (idx < 0) {
            throw new ConnectionConfigError('unknown-project', `no project named "${projectName}" in dbeditor.json`);
