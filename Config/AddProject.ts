 ...
 */
export const addProjectToConfig = (rawConfig: unknown, input: AddProjectInput): Config => {
    ...
    /*
     * Belt-and-braces: re-validate the merged config. If `SchemaConfig`
     * is ever tightened, this catches the resulting incompatibility
     * here rather than at next server boot.
     */
 ...
 * to the patch. Same field validation as `addProjectToConfig`: name
 * uniqueness, schemaPath uniqueness, dialect/mode enum membership.
 * Uniqueness checks ignore the current entry so renaming "Foo" to
 * "FOO" or keeping the schemaPath unchanged are valid no-ops.
 */
export const updateProjectInConfig = (
    rawConfig: unknown,
    projectName: string,
     ...
     * convention matches addProjectToConfig: name = insensitive,
    ...
    /*
     * Reconstruct the project with key order matching how AddProject
 ...
 */
export const removeProjectFromConfig = (rawConfig: unknown, projectName: string): Config => {
    const cfgErrors: SchemaErrors = [];
    if (!SchemaConfig.validate(rawConfig, cfgErrors)) {
        throw new AddProjectError(
            'invalid-config',
