...
/**
 * Output of `_buildFigureData`: per-table effective canvas position
...
type FigureData = {
     ...
     * shape so `_parseTable` can drop the value straight onto the
     * model.
     */
...
export class MwbReader {

    private static readonly _parser: XMLParser = new XMLParser({
    ...
    private static readonly _KNOWN_ATTRS: Set<string> = new Set([
        '@_type', '@_struct-name', '@_id', '@_key', '@_content-type', '@_content-struct-name'
    ]);

    private static _asArray<T>(x: T | T[] | undefined): T[] {
        if (x === undefined) {return [];}
        return Array.isArray(x) ? x : [x];
    }

    private static _textOf(n: GrtNode | GrtLink | undefined): string {
        if (!n) {return '';}
        const t = n['#text'];
     ...
     */
    private static _child(struct: GrtNode, key: string): GrtNode | undefined {
        const values = MwbReader._asArray(struct.value);
        return values.find(v => v['@_key'] === key);
    }

    private static _childLink(struct: GrtNode, key: string): string | undefined {
        const links = MwbReader._asArray(struct.link);
        const direct = links.find(l => l['@_key'] === key);
        if (direct) {return MwbReader._textOf(direct);}
        /*
    ...
    private static _childStr(struct: GrtNode, key: string): string {
        return MwbReader._textOf(MwbReader._child(struct, key));
    }

    /* -- mwb Phase E passthrough -------------------------------------------- */

    private static _xmlEscapeText(s: string): string {
        return s.replace(/&/gu, '&amp;')
            .replace(/</gu, '&lt;')
            .replace(/>/gu, '&gt;');
    }

    private static _xmlEscapeAttr(s: string): string {
        return MwbReader._xmlEscapeText(s).replace(/"/gu, '&quot;');
    }
     ...
     */
    private static _serializeNode(node: GrtNode | GrtLink, kind: 'value' | 'link', depth: number, key?: string): string {
        const pad = '  '.repeat(depth);
        const attrs: string[] = [];
        if (node['@_type']) {attrs.push(`type="${MwbReader._xmlEscapeAttr(node['@_type'])}"`);}
        if ('@_struct-name' in node && node['@_struct-name']) {
            attrs.push(`struct-name="${MwbReader._xmlEscapeAttr(node['@_struct-name'])}"`);
        }
        if ('@_content-type' in node && (node as GrtNode)['@_content-type']) {
            attrs.push(`content-type="${MwbReader._xmlEscapeAttr((node as GrtNode)['@_content-type'] as string)}"`);
        }
        if ('@_content-struct-name' in node && (node as GrtNode)['@_content-struct-name']) {
            attrs.push(`content-struct-name="${MwbReader._xmlEscapeAttr((node as GrtNode)['@_content-struct-name'] as string)}"`);
        }
        if ('@_id' in node && (node as GrtNode)['@_id']) {
            attrs.push(`id="${MwbReader._xmlEscapeAttr((node as GrtNode)['@_id'] as string)}"`);
        }
        if (key !== undefined) {attrs.push(`key="${MwbReader._xmlEscapeAttr(key)}"`);}
        else if (node['@_key']) {attrs.push(`key="${MwbReader._xmlEscapeAttr(node['@_key'])}"`);}
        ...
        if (kind === 'link') {
            ...
            return `${pad}${opener}>${MwbReader._xmlEscapeText(String(text))}</link>\n`;
        }

        const valueNode = node as GrtNode;
        const values = MwbReader._asArray(valueNode.value);
        const links = MwbReader._asArray(valueNode.link);
        ...
        if (values.length || links.length) {
            body += '\n';
            for (const v of values) {body += MwbReader._serializeNode(v, 'value', depth + 1);}
            for (const l of links) {body += MwbReader._serializeNode(l, 'link', depth + 1);}
            body += pad;
        } else {
            body = MwbReader._xmlEscapeText(String(text));
        }
     ...
     */
    private static _capturePassthrough(
        struct: GrtNode,
        consumed: Set<string>,
        childDepth: number
    ): WbPassthrough | undefined {
        const valuesOut: {key: string; xml: string;}[] = [];
        for (const v of MwbReader._asArray(struct.value)) {
            const k = v['@_key'];
            if (!k || consumed.has(k)) {continue;}
            valuesOut.push({key: k, xml: MwbReader._serializeNode(v, 'value', childDepth).trimEnd()});
        }
        for (const l of MwbReader._asArray(struct.link)) {
            const k = l['@_key'];
            if (!k || consumed.has(k)) {continue;}
            valuesOut.push({key: k, xml: MwbReader._serializeNode(l, 'link', childDepth).trimEnd()});
        }
        const attrsOut: {name: string; value: string;}[] = [];
        for (const a of Object.keys(struct)) {
            if (!a.startsWith('@_')) {continue;}
            if (MwbReader._KNOWN_ATTRS.has(a)) {continue;}
    ...
    private static _childInt(struct: GrtNode, key: string): number {
        const t = MwbReader._textOf(MwbReader._child(struct, key));
        if (!t) {return 0;}
        const n = Number(t);
    ...
    /**
     * Like `_childInt` but rounds the real-valued result. Workbench stores
     ...
     */
    private static _childIntRound(struct: GrtNode, key: string): number {
        const t = MwbReader._textOf(MwbReader._child(struct, key));
        if (!t) {return 0;}
        const n = Number(t);
     ...
     */
    private static _findStructs(node: GrtNode | undefined, structName: string, out: GrtNode[] = []): GrtNode[] {
        if (!node) {return out;}
        if (node['@_struct-name'] === structName) {out.push(node);}
        for (const v of MwbReader._asArray(node.value)) {MwbReader._findStructs(v, structName, out);}
        return out;
    }
     ...
     */
    private static _extractObjectXmlByGrtId(xml: string, structName: string): Map<string, string> {
     ...
     */
    private static _mapSimpleType(datatypeRef: string): string {
    ...
    private static _mapFkAction(raw: string): string | undefined {
        const v = (raw || '').toUpperCase().trim();
        if (!v) {return undefined;}
        return v;
    }

    private static _parseColumn(col: GrtNode, tableWbId: string): ColumnRecord {
        const wbId = col['@_id'] ?? '';
        const datatypeRef = MwbReader._childLink(col, 'simpleType') ?? '';
        const typeName = MwbReader._mapSimpleType(datatypeRef);
        const lengthRaw = MwbReader._childInt(col, 'length');
        const precision = MwbReader._childInt(col, 'precision');
        const scale = MwbReader._childInt(col, 'scale');
        const isNotNull = MwbReader._childInt(col, 'isNotNull') === 1;
        const autoInc = MwbReader._childInt(col, 'autoIncrement') === 1;
        const defaultIsNull = MwbReader._childInt(col, 'defaultValueIsNull') === 1;
        const defaultValue = defaultIsNull ? '' : MwbReader._childStr(col, 'defaultValue');
        const comment = MwbReader._childStr(col, 'comment');
        const charset = MwbReader._childStr(col, 'characterSetName');
        const collation = MwbReader._childStr(col, 'collationName');
        /*
         ...
         * and ignored.
         */
        const flagsList = MwbReader._child(col, 'flags');
        const flags = flagsList ? MwbReader._asArray(flagsList.value).map(v => MwbReader._textOf(v).toUpperCase()) : [];
        const unsigned = flags.includes('UNSIGNED');

        const column: JsonColumn = {
            unid: randomUUID(),
            name: MwbReader._childStr(col, 'name'),
            type: typeName
        };
         ...
         * `_childStr/_childInt/_childLink/_child(col, ...)` lookup above plus
        ...
        const pt = MwbReader._capturePassthrough(col, consumed, 3);
     ...
     */
    private static _parseIndex(
        idx: GrtNode,
        columnByWbId: Map<string, ColumnRecord>
    ): JsonIndex | null {
        const isPrimary = MwbReader._childInt(idx, 'isPrimary') === 1;
        const unique = MwbReader._childInt(idx, 'unique') === 1;
        const rawType = MwbReader._childStr(idx, 'indexType').toUpperCase();

        const ixColumnsNode = MwbReader._child(idx, 'columns');
        const wbIndexColumns = ixColumnsNode ? MwbReader._asArray(ixColumnsNode.value) : [];

        const indexColumns: JsonIndexColumn[] = [];
        for (const ic of wbIndexColumns) {
            const refWbId = MwbReader._childLink(ic, 'referencedColumn');
            if (!refWbId) {continue;}
            const ref = columnByWbId.get(refWbId);
            if (!ref) {continue;}
            const desc = MwbReader._childInt(ic, 'descend') === 1;
            const colLen = MwbReader._childInt(ic, 'columnLength');
        ...
        const result: JsonIndex = {
            unid: randomUUID(),
            name: MwbReader._childStr(idx, 'name'),
            type: type,
        ...
        const pt = MwbReader._capturePassthrough(idx, consumed, 3);
        if (pt) {result.wbPassthrough = pt;}
        return result;
    }

    private static _parseForeignKey(
        fk: GrtNode,
        columnByWbId: Map<string, ColumnRecord>,
        tableByWbId: Map<string, TableRecord>
    ): JsonForeignKey | null {
        const refTableWbId = MwbReader._childLink(fk, 'referencedTable');
        if (!refTableWbId) {return null;}
        const refTable = tableByWbId.get(refTableWbId);
        if (!refTable) {return null;}

        const localColumnsNode = MwbReader._child(fk, 'columns');
        const refColumnsNode = MwbReader._child(fk, 'referencedColumns');
        const localLinks = localColumnsNode ? MwbReader._asArray(localColumnsNode.link) : [];
        const refLinks = refColumnsNode ? MwbReader._asArray(refColumnsNode.link) : [];
        ...
        for (let i = 0; i < localLinks.length; i++) {
            const localWb = MwbReader._textOf(localLinks[i]);
            const refWb = MwbReader._textOf(refLinks[i]);
        ...
        const result: JsonForeignKey = {
            unid: randomUUID(),
            name: MwbReader._childStr(fk, 'name'),
            refTableUnid: refTable.table.unid,
            columns: cols
        };
        const del = MwbReader._mapFkAction(MwbReader._childStr(fk, 'deleteRule'));
        const upd = MwbReader._mapFkAction(MwbReader._childStr(fk, 'updateRule'));
        ...
        const pt = MwbReader._capturePassthrough(fk, consumed, 3);
        if (pt) {result.wbPassthrough = pt;}
        return result;
    }
     ...
     * after.
     */
    private static _stripViewPrefix(sql: string): string {
        const m = sql.match(/^\s*CREATE\b[\s\S]*?\bVIEW\b[\s\S]*?\bAS\s+/iu);
        if (!m) {return sql.trim();}
        return sql.substring(m[0].length).trim();
    }

    private static _parseView(
        v: GrtNode,
        viewPositions: Map<string, {x: number; y: number;}>
    ): JsonView {
        const sql = MwbReader._childStr(v, 'sqlDefinition');
        const comment = MwbReader._childStr(v, 'comment');
        const wbId = v['@_id'] ?? '';
        const pos = viewPositions.get(wbId) ?? {x: 80, y: 80};
        const result: JsonView = {
            unid: randomUUID(),
            name: MwbReader._childStr(v, 'name'),
            pos: pos,
            select: MwbReader._stripViewPrefix(sql)
        };
        if (comment) {result.description = comment;}
        const consumed = new Set(['sqlDefinition', 'comment', 'name', 'oldName', 'owner']);
        const pt = MwbReader._capturePassthrough(v, consumed, 3);
        if (pt) {result.wbPassthrough = pt;}
        return result;
    }
     ...
     */
    private static _mapRoutineKind(raw: string): string {
    ...
    private static _parseRoutine(r: GrtNode): JsonRoutine {
        const comment = MwbReader._childStr(r, 'comment');
        const result: JsonRoutine = {
            unid: randomUUID(),
            name: MwbReader._childStr(r, 'name'),
            pos: {x: 80, y: 80},
            kind: MwbReader._mapRoutineKind(MwbReader._childStr(r, 'routineType')),
            body: MwbReader._childStr(r, 'sqlDefinition')
        };
        if (comment) {result.description = comment;}
        const consumed = new Set(['sqlDefinition', 'comment', 'name', 'oldName', 'owner', 'routineType']);
        const pt = MwbReader._capturePassthrough(r, consumed, 3);
        if (pt) {result.wbPassthrough = pt;}
        return result;
    }

    private static _parseTrigger(t: GrtNode): JsonRoutine {
        const comment = MwbReader._childStr(t, 'comment');
        const result: JsonRoutine = {
            unid: randomUUID(),
            name: MwbReader._childStr(t, 'name'),
            pos: {x: 80, y: 80},
            kind: JsonRoutineKind.trigger,
            body: MwbReader._childStr(t, 'sqlDefinition')
        };
        if (comment) {result.description = comment;}
        const consumed = new Set(['sqlDefinition', 'comment', 'name', 'oldName', 'owner']);
        const pt = MwbReader._capturePassthrough(t, consumed, 3);
        if (pt) {result.wbPassthrough = pt;}
        return result;
    }

    private static _parseTable(
        tbl: GrtNode,
        columnByWbId: Map<string, ColumnRecord>,
        ...
        const wbId = tbl['@_id'] ?? '';
        const name = MwbReader._childStr(tbl, 'name');
        const comment = MwbReader._childStr(tbl, 'comment');
        const engine = MwbReader._childStr(tbl, 'tableEngine');
        const charset = MwbReader._childStr(tbl, 'defaultCharacterSetName');
        const collation = MwbReader._childStr(tbl, 'defaultCollationName');

        const columnsNode = MwbReader._child(tbl, 'columns');
        const wbColumns = columnsNode ? MwbReader._asArray(columnsNode.value) : [];
        const columns: JsonColumn[] = [];
        const columnWbIds: string[] = [];
        for (const c of wbColumns) {
            const rec = MwbReader._parseColumn(c, wbId);
         ...
         * by the caller after _parseColumn fills columnByWbId — those keys
         * are also consumed but flow through different code paths, so we
         * mark them consumed here too so _capturePassthrough doesn't
         * double-store them.
         */
        /*
        ...
        const pt = MwbReader._capturePassthrough(tbl, consumed, 3);
     ...
     */
    private static _buildFigureData(root: GrtNode): FigureData {
        const tableFigures = MwbReader._findStructs(root, 'workbench.physical.TableFigure');
        const viewFigures = MwbReader._findStructs(root, 'workbench.physical.ViewFigure');

        /*
         * Index Workbench diagrams by ID so we can recover their `name`
         * for the synthesised layers + walk their authored child layers.
         * `_findStructs` walks document order, so insertion order is
         * stable.
         */
        const diagrams = MwbReader._findStructs(root, 'workbench.physical.Diagram');
        ...
        for (const d of diagrams) {
            ...
            if (id) {
                ...
                diagramName.set(id, MwbReader._childStr(d, 'name'));
            }
        }
        ...
        for (const fig of tableFigures) {
            const owner = MwbReader._childLink(fig, 'owner') ?? 'unknown';
        ...
        for (const [diagramId, figs] of byDiagram) {
            ...
            for (const entry of figs) {
                const left = MwbReader._childIntRound(entry.fig, 'left');
                const width = MwbReader._childIntRound(entry.fig, 'width') || FALLBACK_WIDTH;
            ...
            if (diagramNode) {
                const allLayers = MwbReader._findStructs(diagramNode, 'workbench.physical.Layer');
                for (const l of allLayers) {
                    if (l['@_key'] === 'rootLayer') {continue;}
                    outLayers.push({
                        unid: randomUUID(),
                        name: MwbReader._childStr(l, 'name') || `Layer ${outLayers.length + 1}`,
                        diagramUnid: diagramUnid,
                        pos: {
                            x: MwbReader._childIntRound(l, 'left') + xShift,
                            y: MwbReader._childIntRound(l, 'top')
                        },
                        width: MwbReader._childIntRound(l, 'width') || 400,
                        height: MwbReader._childIntRound(l, 'height') || 300,
                        color: MwbReader._childStr(l, 'color') || undefined
                    });
                }
            }
            ...
            for (const entry of figs) {
                if (entry.kind === 'table') {
                    const tableWbId = MwbReader._childLink(entry.fig, 'table');
                    if (!tableWbId) {continue;}
                    const figPos = {
                        x: MwbReader._childIntRound(entry.fig, 'left') + xShift,
                        y: MwbReader._childIntRound(entry.fig, 'top')
                    };
                    ...
                    const viewWbId = MwbReader._childLink(entry.fig, 'view');
    ...
    public static parse(buffer: Buffer): MwbImportResult {
         ...
         * lists), flatten via `_findStructs`. The shape is unique enough
         * that this is unambiguous in practice.
         */
        const grtRoot = root as unknown as GrtNode;
        const schemas = MwbReader._findStructs(grtRoot, 'db.mysql.Schema');
        const figureData = MwbReader._buildFigureData(grtRoot);
         ...
         * scope.
         */
        const routineXmlByWbId = MwbReader._extractObjectXmlByGrtId(xml, 'db.mysql.Routine');
        const routineOriginalXml = new Map<string, string>();
        const viewXmlByWbId = MwbReader._extractObjectXmlByGrtId(xml, 'db.mysql.View');
         ...
         */
        const tableXmlByWbId = MwbReader._extractObjectXmlByGrtId(xml, 'db.mysql.Table');
        ...
        for (const schema of schemas) {
            const schemaName = MwbReader._childStr(schema, 'name') || 'imported';
            const tablesNode = MwbReader._child(schema, 'tables');
            const wbTables = tablesNode ? MwbReader._asArray(tablesNode.value) : [];
            ...
            for (const t of wbTables) {
                const rec = MwbReader._parseTable(t, columnByWbId, figurePos, tableToLayer, tablePlacementsMap);
            ...
            for (const rec of records) {
                ...
                if (!wbTable) {continue;}
                const indexesNode = MwbReader._child(wbTable, 'indices');
                const wbIndexes = indexesNode ? MwbReader._asArray(indexesNode.value) : [];
                for (const ix of wbIndexes) {
                    const parsed2 = MwbReader._parseIndex(ix, columnByWbId);
                    if (parsed2) {rec.table.indexes.push(parsed2); indexCount++;}
                }
                const fksNode = MwbReader._child(wbTable, 'foreignKeys');
                const wbFks = fksNode ? MwbReader._asArray(fksNode.value) : [];
                for (const fk of wbFks) {
                    const parsed2 = MwbReader._parseForeignKey(fk, columnByWbId, tableByWbId);
                    if (parsed2) {rec.table.foreignKeys.push(parsed2); fkCount++;}
                }
                const triggersNode = MwbReader._child(wbTable, 'triggers');
                const wbTriggers = triggersNode ? MwbReader._asArray(triggersNode.value) : [];
                for (const tg of wbTriggers) {
                    triggersForSchema.push(MwbReader._parseTrigger(tg));
                    triggerCount++;
                }
            }

            const viewsNode = MwbReader._child(schema, 'views');
            const wbViews = viewsNode ? MwbReader._asArray(viewsNode.value) : [];
            const views: JsonView[] = [];
            for (const v of wbViews) {
                const parsedView = MwbReader._parseView(v, viewFigurePos);
            ...
            const routinesNode = MwbReader._child(schema, 'routines');
            const wbRoutines = routinesNode ? MwbReader._asArray(routinesNode.value) : [];
            const routines: JsonRoutine[] = [];
            for (const r of wbRoutines) {
                const routine = MwbReader._parseRoutine(r);
             ...
             */
            const defaultCharset = MwbReader._childStr(schema, 'defaultCharacterSetName');
            const defaultCollation = MwbReader._childStr(schema, 'defaultCollationName');
            ...
            const schemaPt = MwbReader._capturePassthrough(schema, consumedSchemaKeys, 4);
    ...
    /**
     * Convenience: wrap `parse` output into a root-level `JsonDataDB`
     * suitable for `replaceFs`. Imported databases become direct children
     * of the synthetic root node.
     */
    public static parseAsFsRoot(buffer: Buffer): { fs: JsonDataDB; stats: MwbImportResult; } {
        const stats = MwbReader.parse(buffer);
        const fs: JsonDataDB = {
            unid: 'root',
            name: 'root',
