...
let _instance: BrowserJsPlumbInstance | null = null;

/**
 * Compute a `[side, yRatio, dx, 0]` anchor on the table card that contains
 * `rowEl`. The y position is the centre of the row relative to the card.
 * `side` defaults to `right`; the persisted FK render pass flips it later
 * based on the actual relative position of the two cards, so the draft
 * looking slightly off mid-drag is acceptable. Returns `null` when the
 * row's enclosing `.db-table` can't be located.
 */
const anchorForRow = (rowEl: Element, side: 'left' | 'right' = 'right'): [number, number, number, number] | null => {
 ...
 *
 * Source/target selectors are registered once here so any column row
 * rendered later (after a re-render) is automatically draggable into a
 * new connection — selectors are matched dynamically by CSS class.
 */
export const getJsPlumbInstance = (): BrowserJsPlumbInstance => {
    if (_instance) {
        return _instance;
    }
    /*
     ...
     * element (the grip on source, the column row on target) and writes
     * them to `endpoint.parameters` via `mergeParameters` — that's the
     * documented path to surface DOM data on the connection event.
     */
    const EXTRACT = {
    ...
    /*
     * Source: row-anchored draft line. The grip's row centre becomes the
     * anchor's y; side defaults to right (the grip lives on the right edge
     * of the card). Falling back to `Continuous` keeps the old behaviour
     * if measurement ever fails.
     */
    _instance.addSourceSelector('.db-table-column-grip', {
        anchor: 'Continuous',
        anchorPositionFinder: (el: Element): [number, number, number, number] | null =>
            anchorForRow(el, 'right'),
    ...
    _instance.addTargetSelector('.db-table-column', {
        anchor: 'Continuous',
        anchorPositionFinder: (el: Element): [number, number, number, number] | null =>
            anchorForRow(el, 'left'),
        endpoint: 'Blank',
        edgeType: 'fk-draft',
        extract: EXTRACT
    } as Record<string, unknown>);
    ...
    /*
     * Target: drop on the card itself (NOT a specific column row). Used
     * by the auto-column flow — when the user drops a FK draft onto a
     * table card without aiming at a row, we create the matching column
     * server-side and then create the FK. The selector deliberately
     * matches the card header so we don't hijack drops inside the
     * column-list area (where the row-level selector takes precedence).
     *
     * The header carries `data-table-unid` via the parent `.db-table` —
     * we extract it through the closest-ancestor lookup in `extract`.
     * Falling back to `Continuous` anchor for the visual.
     */
    _instance.addTargetSelector('.db-table-header', {
        anchor: 'Continuous',
        anchorPositionFinder: (el: Element): [number, number, number, number] | null =>
            anchorForRow(el, 'left'),
        endpoint: 'Blank',
        ...
        extract: {
            /*
             * The header doesn't itself have `data-table-unid` (that's on
             * the parent `.db-table` card), but jsPlumb's `extract` reads
             * direct attributes only. We surface the table unid via the
             * connection event by reading the closest ancestor in
             * `_bindJsPlumb` instead — see the fallback there.
             */
            'data-table-unid': 'tableUnid'
        }
    } as Record<string, unknown>);
...
export const resetJsPlumbInstance = (): void => {
    if (_instance) {
        _instance.destroy();
        _instance = null;
    }
