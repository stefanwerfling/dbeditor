 ...
 * understood by EnvPlaceholderResolver — and not an accidentally-untemplated
...
describe('resolveEnvPlaceholders', () => {
    ...
    it('escapes $$ to a literal $', () => {
        const out = resolveEnvPlaceholders({s: '$${VAR}'}, {VAR: 'should-not-be-used'});
    ...
    it('does not mutate the input object', () => {
        ...
        resolveEnvPlaceholders(input, {HOST: 'db.local'});
        expect(input).toEqual({host: '${HOST}'});
    });

    it('leaves strings without placeholders unchanged', () => {
        const out = resolveEnvPlaceholders({name: 'plain-string'}, {});
