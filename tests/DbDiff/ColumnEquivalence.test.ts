...
import {
    ...
    viewsEquivalent
} from '../../DbDiff/ColumnEquivalence.js';
import {
    JsonColumn,
    JsonForeignKey,
    JsonIndex,
...
describe('diffColumn', () => {
    ...
    it('returns null for identical columns', () => {
        ...
        expect(diffColumn(a, b, new Set())).toBeNull();
    });
    ...
    it('strips length on boolean / bool so live-vs-model length difference is ignored', () => {
        ...
        expect(diffColumn(c, d, new Set())).toBeNull();
    });

    it('detects a type change', () => {
        const a = col({type: 'int'});
        const b = col({type: 'bigint'});
        const d = diffColumn(a, b, new Set());
    ...
    it('detects notNull / primaryKey / autoIncrement / unique / unsigned changes', () => {
        ...
        for (const flag of ['notNull', 'primaryKey', 'autoIncrement', 'unique', 'unsigned'] as const) {
            ...
            const d = diffColumn(base, changed, new Set());
    ...
    it('honours ignore set for collation and charset', () => {
        ...
        expect(diffColumn(a, b, new Set(['collation', 'charset']))).toBeNull();
        const d = diffColumn(a, b, new Set());
...
describe('indexColumnNamesEqual', () => {
    ...
    it('matches column unids via the resolver to names', () => {
        ...
        expect(indexColumnNamesEqual(a, b, live, model)).toBe(true);
    });
    ...
    it('detects a renamed column position', () => {
        ...
        expect(indexColumnNamesEqual(a, b, live, model)).toBe(false);
    });
...
describe('fksEquivalent', () => {
    ...
    it('treats matching FKs as equivalent', () => {
        ...
        expect(fksEquivalent(a, b, liveCol, modelCol, liveTbl, modelTbl)).toBe(true);
    });
    ...
    it('detects an onDelete change', () => {
        ...
        expect(fksEquivalent(a, b, liveCol, modelCol, liveTbl, modelTbl)).toBe(false);
    });
    ...
    it('detects a referenced-table change', () => {
        const a = fk({});
        const b = fk({columns: [{columnUnid: 'M1', refColumnUnid: 'M9'}]});
        expect(fksEquivalent(a, b, liveCol, modelCol, liveTbl, (): string => 'accounts')).toBe(false);
    });
...
describe('tableOptionsEquivalent', () => {
    ...
    it('treats undefined and {} as equal', () => {
        expect(tableOptionsEquivalent(undefined, {}, new Set())).toBe(true);
    });

    it('case-folds engine names', () => {
        expect(tableOptionsEquivalent(opt({engine: 'innodb'}), opt({engine: 'InnoDB'}), new Set())).toBe(true);
    });

    it('detects a charset change', () => {
        expect(tableOptionsEquivalent(opt({charset: 'utf8'}), opt({charset: 'utf8mb4'}), new Set())).toBe(false);
    });

    it('honours ignore set', () => {
        expect(tableOptionsEquivalent(
            opt({charset: 'utf8'}),
            opt({charset: 'utf8mb4'}),
            new Set(['charset'])
        )).toBe(true);
...
describe('tableOptionsEquivalent — modelDefaults inheritance', () => {
    ...
    it('treats unset model collation as inherited from modelDefaults.collation', () => {
        ...
        const ok = tableOptionsEquivalent(live, model, new Set(), {collation: 'utf8mb4_unicode_ci'});
        expect(ok).toBe(true);
    });
    ...
    it('falls back for engine + charset the same way', () => {
        ...
        const model = {};
        const ok = tableOptionsEquivalent(live, model, new Set(), {engine: 'InnoDB', charset: 'utf8mb4'});
        expect(ok).toBe(true);
    });
...
describe('viewsEquivalent', () => {
    ...
    it('detects a materialized flag change', () => {
        expect(viewsEquivalent(view({materialized: false}), view({materialized: true}))).toBe(false);
    });

    it('treats undefined materialized as false', () => {
        expect(viewsEquivalent(view({}), view({materialized: false}))).toBe(true);
    });

    it('detects a select-body difference', () => {
        expect(viewsEquivalent(view({select: 'SELECT 1'}), view({select: 'SELECT 2'}))).toBe(false);
    });

    it('ignores leading/trailing whitespace in select body', () => {
        expect(viewsEquivalent(view({select: '  SELECT 1  '}), view({select: 'SELECT 1'}))).toBe(true);
    });
