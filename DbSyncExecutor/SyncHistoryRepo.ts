 ...
 */
export const historyPathFor = (schemaPath: string): string => {
 ...
 * leave the file untouched. The next successful append will replace
 * the file atomically and re-establish the canonical shape.
 */
export const loadHistory = (historyPath: string): SyncHistoryEntry[] => {
...
/**
 * Append a new entry to the front of the history (newest first) and
 * persist via atomic tmp+rename. Returns the entry with `id` and
 * `ts` filled in (so callers don't need to mint them).
 *
...
export const appendEntry = (
    ...
    try {
        const existing = loadHistory(historyPath);
