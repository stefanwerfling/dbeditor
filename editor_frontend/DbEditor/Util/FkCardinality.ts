...
export class FkCardinality {
    ...
    private static _setsEqual<T>(a: Set<T>, b: Set<T>): boolean {
    ...
    private static _isUniqueIndex(ix: JsonIndex): boolean {
     ...
     * over `fkColumnUnids`.
     */
    public static isOneToOne(table: JsonTable, fkColumnUnids: string[]): boolean {
        ...
        if (pkUnids.size > 0 && FkCardinality._setsEqual(pkUnids, fkSet)) {return true;}
        ...
        for (const ix of table.indexes) {
            if (!FkCardinality._isUniqueIndex(ix)) {continue;}
            const ixUnids = new Set(ix.columns.map(ic => ic.columnUnid));
            if (FkCardinality._setsEqual(ixUnids, fkSet)) {return true;}
        }
        ...
        if (fkColumnUnids.length === 1) {
            const c = table.columns.find(x => x.unid === fkColumnUnids[0]);
            if (c?.unique === true) {return true;}
        }
