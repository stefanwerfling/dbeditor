import {JsonColumn, JsonDataDB, JsonEnum, JsonTable} from '../../editor_schemas/JsonData.js';
import {DbFsTreeWalker} from '../DbRepository/DbFsTreeWalker.js';
import {DialectContext} from './DbDialect.js';

export class DialectContextBuilder {

    /**
     * Builds a `DialectContext` from a model tree. The context exposes
     * cross-table lookups (FK targets, enum types referenced by columns)
     * by `unid`. Both the file/migration codegen and the sync codegen
     * share this helper so context shape never diverges between them.
     */
